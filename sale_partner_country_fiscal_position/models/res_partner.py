...
class ResPartner(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get('country_id') and not \
                ...
                if int(vals['country_id']) == self.env['res.country'].search(
                    ...
                    vals['property_account_position_id'] = \
                        self.env.ref(account_position_with_vat.complete_name).id
                elif int(vals['country_id']) in self.env.ref('base.europe')\
                        .country_ids.ids:
                    vals['property_account_position_id'] = \
                        self.env.ref(account_position_y.complete_name).id
                else:
                    vals['property_account_position_id'] = \
                            self.env.ref(account_position_ey.complete_name).id
